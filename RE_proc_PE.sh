...
cd /storage/plzen1/home/sharaf/repex/reads_4_repex/

fbname=$(basename "$FW" .)
fbname=${fbname%_1.fq.gz*}
...
java -jar /software/trimmomatic/0.36/dist/jar/trimmomatic-0.36.jar PE -threads 8 -phred33 $FW $RV -baseout $fbname/$fbname.fq.gz LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 HEADCROP:$HC CROP:$EC MINLEN:$EC
...
zcat $fbname/*_1P.fq.gz | sed -n '1~4s/^@/>/p;2~4p' > $fbname/$fbname.1P.fasta

sed "s/\// /g;s/:/_/g;s/1_N.*/ 1/g;s/ .*/ 1/g" $fbname/$fbname.1P.fasta >> $fbname/$fbname.1P.renamed.fasta

zcat $fbname/*_2P.fq.gz | sed -n '1~4s/^@/>/p;2~4p' > $fbname/$fbname.2P.fasta

sed "s/\// /g;s/:/_/g;s/2_N.*/ 2/g;s/ .*/ 2/g" $fbname/$fbname.2P.fasta >> $fbname/$fbname.2P.renamed.fasta

#interlacing

/storage/brno7-cerit/home/galaxyelixir/galaxy/tools/umbr_utils/utils/fasta_interlacer -a $fbname/$fbname.1P.renamed.fasta -b $fbname/$fbname.2P.renamed.fasta -p $fbname/$fbname.1P2P.interlaced.fasta -x $fbname/$fbname.singles.fasta
